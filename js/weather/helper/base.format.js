var checker = require('./base.checker');
var strFormat = require('string-format');

var BaseFormat = function () {};
BaseFormat.prototype = Object.assign({}, BaseFormat.prototype, {
  ...
  toCapitalizeStr: function (str) {
    return str.charAt(0).toUpperCase() + str.slice(1);
  },

  toDeCapitalizeStr: function (str) {
    return str.charAt(0).toLowerCase() + str.slice(1);
  },

  toCamelStr: function (str, isCaptalize) {
    var _this = this;
    var res = str.split(/[-_]/).map(function (token) {
      return _this.toCapitalizeStr(token);
    }).join('');
    if (isCaptalize !== true) {
      res = this.toDeCapitalizeStr(res);
    }
    return res;
  },
  ...
  toISOStr: function (date) {
    return date.toISOString();
  },

  formatDbDate: function (dateStr) {
    return this.formatDate(new Date(dateStr), 'yyyy-MM-dd HH:mm:ss');
  },

  formatShortDate: function (dateStr) {
    return this.formatDate(new Date(dateStr), 'MM/dd HH:mm:ss');
  },
  ...
  toHexStr: function (num, numdigits) {
    var hexStr = num.toString(16);
    while (hexStr.length < numdigits) {
      hexStr = '0' + hexStr;
    }
...
module.exports = exports = new BaseFormat();